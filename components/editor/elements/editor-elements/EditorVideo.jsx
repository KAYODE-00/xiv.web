...
const EditorVideo = ({ element }) => {
  ...
  React.useEffect(() => {
    return setupScrollAnimation(videoRef.current, deviceStyles, isLive || isPreview);
  }, [
  ...
  return (
    ...
    >
      ...
      {!Array.isArray(element.content) && element.content.src ? (
        <iframe
          width={deviceStyles.width || "560"}
          height={deviceStyles.height || "315"}
        ...
        !isLive && (
          <div className="w-full h-48 bg-[#111118]
            border border-dashed border-[#1e1e2e]
            rounded flex flex-col items-center
            justify-center text-gray-600 gap-2">
            <div className="text-3xl">▶</div>
            <p className="text-xs">Add video URL in settings</p>
          </div>
        )
      )}
